...
MODELS_DICT = {
    ...
    'Time Series': TSForecastingExperiment(),
}
...
class Data:
    ...
    def upload_file(self):
        ...
        if uploaded_file:
            ...
            object_columns = self.dataframe.select_dtypes('object').columns
            ...
            self.problem_type = st.sidebar.selectbox('Please, choose the problem type.', 
            ...
            if self.problem_type == 'Time Series':
                ...
                for column in object_columns:
                    try:
                        pd.to_datetime(self.dataframe[column])
                        if self.dataframe[column].apply(len).mean() >= 10:
                            potential_date_columns.append(column)
                    except:
                        continue

                if len(potential_date_columns) == 1:
                    self.date_column = potential_date_columns[0]
                elif len(potential_date_columns) == 0:
                    self.date_column = None
                else:
                    self.date_column = st.selectbox('Choose the Date column', potential_date_columns)
                
                if self.date_column:
                    # TODO сгруппировать для агрегации
                    if self.dataframe[self.date_column].nunique() < self.dataframe.shape[0]:
                        st.warning('The dataset will be aggregated (Summarized) due to ununique dates.', icon="⚠️")
                        st.info('Smart aggregation will be added soon.', icon="ℹ️")
                        self.dataframe = self.dataframe.groupby(self.date_column).sum()
                else:
                    st.warning('No Date columns were found. Please choose another problem type.', icon="⚠️")
...
def main():
    ...
    if data.dataframe is not None:
        ...
        if button:
            if data.problem_type != 'Time Series':
                data.make_corr_plot()
                data.make_hist_plot()
                ...
                st.info('Baseline for Time Series will be in the upcoming release.', icon="ℹ️")
